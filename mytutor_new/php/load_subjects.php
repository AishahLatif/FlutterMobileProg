...
$numofpage = ceil($number_of_result / $results_per_page);
...
if ($result->num_rows > 0) {
    ...
    $response = array('status' => 'success', 'pageno'=>"$pageno", 'numofpage'=>"$numofpage", 'data'=>$subjects);
    sendJsonResponse($response);
} else {
    $response = array('status' => 'failed', 'pageno'=>"$pageno", 'numofpage'=>"$numofpage", 'data'=>null);
    sendJsonResponse($response);
}
